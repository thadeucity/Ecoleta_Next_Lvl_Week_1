...
const CreatePoint = () => {
  ...
  const [statesShort, setStatesShort] = useState<string[]>([]);
  ...
  useEffect(() => {
    axios.get<IBGEStateResponse[]>('https://servicodados.ibge.gov.br/api/v1/localidades/estados')
      .then(response => {
        const stateAbbreviations = response.data.map(state => state.sigla);

        setStatesShort(stateAbbreviations);
      })
  }, []);
